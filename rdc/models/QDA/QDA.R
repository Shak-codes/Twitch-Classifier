setwd("~/GitHub/Twitch-Classifier/rdc/models/QDA")
source("../utils.R")
library(ROSE)
library(caret)
...
fit_qda <- function(features, response, alpha_values = seq(0, 1, 0.5), k = 10, pca_thresh = 0.75) {
  library(MASS)
  library(caret)
...
qda_model <- fit_qda(features, response)
saveRDS(qda_model, file = "qda_model.rds")
