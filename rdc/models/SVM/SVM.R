setwd("~/GitHub/Twitch-Classifier/rdc/models/SVM")
source("../utils.R")

fit_svm <- function(features, response, k = 10, pca_thresh = 0.95) {
  library(caret)
  ...
  for (i in seq_along(folds)) {
    ...
    # --- Train SVM model with radial kernel and cross-validation ---
    ...
    # --- Predict ---
    predictions <- predict(svm_model, newdata = X_test_pca)
...
for (folder in folders) {
  for (shuffle in shuffled) {
    for (length in lengths) {
      ...
      svm_model <- fit_svm(features, response)
      saveRDS(svm_model, file = paste0("svm", folder, length, shuffle, ".rds"))
      print(paste0("Finished ", paste0("svm", folder, length, shuffle, ".rds")))
    }
  }
}
